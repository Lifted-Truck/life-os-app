...
import dashboard.overview_data as OD   # canonical home of the heartbeat helper


def test_health_sync_age_none_when_no_heartbeat(monkeypatch, tmp_path):
    monkeypatch.setattr(OD, "sync_heartbeat_path", lambda: tmp_path / "nope")
    h = app.health()
    assert h["status"] == "ok"
...
def test_health_sync_ok_when_recent(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(OD, "sync_heartbeat_path", lambda: hb)
    h = app.health()
