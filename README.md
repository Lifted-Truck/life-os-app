...
|---------|--------------|
| `/plan` | Recompute today, show the blocks, and attach check-in buttons for the current (or next) task block. |
...
per fixed-size block from the day template in `scheduler/constants.DEFAULT_BLOCKS`.
...
- **Script-owned files** (the only files this app writes): `daily/README.md`,
  ...
  value updates). Everything else in the data tree belongs to authoring sessions.
